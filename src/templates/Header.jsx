...
import { faBars, faCaretDown, faCaretSquareDown, faCaretSquareUp, faCaretUp, faTimes } from '@fortawesome/free-solid-svg-icons';
        ...
        return(
            <header className="fixed top-0 z-50 w-full">
                <div className="flex flex-row md:items-center items-start justify-items-stretch w-full md:max-w-7xl mx-auto shadow-lg bg-white md:py-5 py-4 z-30">
                    <div className="md:flex-none text-xl mx-5 font-medium text-red-800">
                        <Link to="/">Jmenuji se Mari</Link>
                    </div>
                    <Nav 
                        id="mainMenu" 
                        title="Hlavní menu" 
                        className={"fixed md:contents top-14 z-10 md:top-0 px-5 md:px-0 mt-1 md:mt-0 bg-gray-800 bg-opacity-90 md:bg-none w-screen md:w-auto md:flex-grow md:mx-5 md:visible " + (this.state.showMainMenu ? "visible" : "invisible")} 
                        titleClass="sr-only" ulClass="flex flex-col md:flex-row md:items-center items-start" 
                        liClass="md:mx-2 my-2 md:my-0 uppercase font-normal text-gray-300 md:text-gray-700 md:text-sm hover:text-red-800 hover:underline"                        
                    >
                        <Link to="/">Domů</Link>
                    ...
                    />
                    <button id="mainMenuToggle" className="md:hidden mx-7 text-gray-800 text-lg">
